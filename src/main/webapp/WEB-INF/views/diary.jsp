...
for (DiaryDTO d : list) {%>
	...
	temp+='<a href="#" class="com_under" onclick="update_text(this)">수정</a> | ';
	temp+='<a href="#" class="com_under">삭제</a>';
	temp+='</div>';
	temp+='</div>';
	temp+='<textarea name="" id="" class="comment_" onkeydown="resize(this)" onkeyup="resize(this)" readonly><%=dc.getDc_text()%></textarea>';
	temp+='</div>';
	wrap.innerHTML+=temp;
<%cnt++;}
}%>
